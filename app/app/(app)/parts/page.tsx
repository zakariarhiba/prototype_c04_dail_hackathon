  ...
  return (
    <div className="space-y-8">
      <div className="entete-page">
        <h1 className="text-xl font-semibold">Parts &amp; QR</h1>
        <ResetButton />
      </div>

      <section className="carte space-y-3">
        <h2 className="font-medium">Add new part</h2>
        <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>
          Creates an inventory-master record and generates its QR. Prototype
          identifier only — not a real GS1/barcode standard.
        </p>
        <form className="champ-groupe max-w-md" onSubmit={submitAddPart}>
          ...
          <button className="bouton bouton--sombre" type="submit" disabled={addBusy}>
            {addBusy ? "Saving..." : "Add part + generate QR"}
          </button>
        </form>
      </section>

      <section className="carte space-y-3">
        <h2 className="font-medium">Simulate outbound scan</h2>
        <span className="etiquette-statut etiquette-statut--attention">Simulated — no real scanner/camera</span>
        <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>
          Scan a part&apos;s QR against an open PO. Feeds the same classify/
          confirm flow as the Receiving page — go there next to resolve it.
        </p>
        <form className="champ-groupe max-w-md" onSubmit={submitScan}>
          ...
          <div className="champ">
            <label>Quantity on this delivery</label>
            <input
              type="number"
              min={1}
              value={scanQty}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {parts?.map((p) => (
            <div key={p.id} className="carte space-y-2" style={{ background: "var(--color-bg)" }}>
              ...
              <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>
                Starting quantity: {p.quantity_on_hand} (set at creation — see Inventory for
                received-to-date)
              </p>
            </div>
          ))}
